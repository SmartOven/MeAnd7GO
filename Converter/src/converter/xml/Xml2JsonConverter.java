...
public class Xml2JsonConverter implements Converter<XmlElement, JsonObject> {
    ...
    private JsonValue convertValue(XmlElement xml) {
        ...
        if (xml.value == null) {
            value = new JsonNull();
        } else if (xml.value instanceof XmlSimpleValue simpleValue) {
            value = new JsonString(simpleValue.value);
        } else if (xml.value instanceof final XmlElement element) {
            value = convert(element);
        } else if (xml.value instanceof final XmlElements elements) {
            if (isArray(elements)) {
